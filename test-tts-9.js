...
async function test() {
    ...
    // We already know StartConnection works with 0b0100 and Event number.
    // What if StartSession FAILS because it needs Sequence ID?
    function buildReq(ev, obj, flags, includeSeq) {
        ...
        if (includeSeq) {
            ...
            new DataView(seqb).setInt32(0, 1, false);
            optBufs.push(seqb);
        }
    ...
    tts.onOpen = () => {
        ...
        tts.ws.onmessage = (event) => {
           ...
           if (type === 0b1111) {
               ...
               const size = v.getUint32(8); // roughly
